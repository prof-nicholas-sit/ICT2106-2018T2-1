...
namespace DBLayer.DAL
{
    public class DbObj
    {
        ...
        // for reading
        private static IMongoCollection<Account> _userCollectionModel;
        ...
        public DbObj()
        {
            ...
            _userCollectionModel = _database.GetCollection<Account>("User_Account"); 
        ...
        public IEnumerable SelectWhere<T>(Type type, string field, T value)
        {
            ...
            if (type == typeof(Account))
            {
                ...
                var filter = Builders<Account>.Filter.Eq(field, value);
        ...
        public async void Commit<T>(GenericSet<T> set) where T : class
        {
            ...
            if (typeof(T) == typeof(Account))
            {
                ...
                // UPDATE
                foreach (var doc in set.DirtyList as List<Account>)
                {
                ...
                // DELETE
                foreach (var doc in set.RemovedList  as List<Account>)
                {
